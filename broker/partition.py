...
class Partition:
    ...
    def append(self, key, value):
        with self.lock:
            payload = {"offset": self.next_offset, "key": key, "value": value}
            record = {"version": 1, "type": "produce", "payload": payload}
            self.storage_engine.append_message(self.log_file_path, record)
            self.next_offset += 1
            return payload
